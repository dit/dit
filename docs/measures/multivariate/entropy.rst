...
Let's consider two coins that are interdependent: the first coin fips fairly,
and if the first comes up heads, the other is fair, but if the firt comes up
tails the other is certainly tails:

.. ipython::

   In [1]: d = dit.Distribution(['HH', 'HT', 'TT'], [1/4, 1/4, 1/2])

We would expect that entropy of the second coin conditioned on the first coin
would be :math:`0.5` bits, and sure enough that is what we find:
...
And since the first coin is fair, we would expect it to have an entropy of
:math:`1` bit:
...
Below we have a pictoral representation of the joint entropy for both 2 and 3
