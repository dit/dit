...
def test_sample6():
    ...
    d = dit.example_dists.Xor()
    with pytest.raises(ditException):
        module.sample(d, 5, rand=[0.1] * 3)


def test_sample_discrete_python1():
    # Specified rand number
    d = dit.example_dists.Xor()
    x = module._sample_discrete__python(d.pmf, 0.5)
    assert x == 2
...
def test_sample_discrete_python2():
    # Specified rand number
    d = dit.example_dists.Xor()
    x = module._samples_discrete__python(d.pmf, np.array([0.5, 0.3, 0.2]))
...
def test_sample_discrete_python3():
    ...
    d = dit.example_dists.Xor()
    out = np.zeros((3,))
    module._samples_discrete__python(d.pmf, np.array([0.5, 0.3, 0.2]), out=out)
...
def test_norm_diagonal_cov():
    d = np.array([0.2, 0.3, 0.5])
    dit.math.prng.seed(0)
    x = dit.math.norm(d, np.array([0.3, 0.5]))
...
def test_norm_badshape_cov():
    d = np.array([0.2, 0.3, 0.5])
    dit.math.prng.seed(0)
    ilrcov = np.array([[0.3, 0.5, 0.1], [0.5, 0.4, 0.3], [0.2, 0.5, 0.3]])
    with pytest.raises(ditException):
        dit.math.norm(d, ilrcov)

    ilrcov = np.array([0.3, 0.5, 0.1])
    with pytest.raises(ditException):
        dit.math.norm(d, ilrcov)
...
def test_norm_toomany():
    d = np.array([[0.2, 0.3, 0.5], [0.5, 0.2, 0.3]])
    with pytest.raises(ditException):
        dit.math.norm(d)
