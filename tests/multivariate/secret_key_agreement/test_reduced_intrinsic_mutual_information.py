...
def test_1(dist, val, backend):
    ...
    """
    rimi = reduced_intrinsic_total_correlation(dist, [[0], [1]], [2], bound=(4,), backend=backend)
...
def test_2(dist, val, backend):
    ...
    Test against known values.
    """
    rimi = reduced_intrinsic_dual_total_correlation(dist, [[0], [1]], [2], bound=(4,), backend=backend)
    assert rimi == pytest.approx(val, abs=1e-5)
...
def test_3(dist, val, backend):
    ...
    Test against known values.
    """
    rimi = reduced_intrinsic_CAEKL_mutual_information(dist, [[0], [1]], [2], bound=(4,), backend=backend)
    assert rimi == pytest.approx(val, abs=1e-5)
