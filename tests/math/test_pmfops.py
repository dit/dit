...
def test_downsample_twopmf():
    # Two pmf
    d1 = np.array([[0.0, 0.51, 0.49], [0.6, 0.3, 0.1]])
    d2_ = np.array([[0.0, 0.5, 0.5], [0.5, 0.5, 0.0]])
    d2 = module.downsample(d1, 2)
    assert np.allclose(d2, d2_)
