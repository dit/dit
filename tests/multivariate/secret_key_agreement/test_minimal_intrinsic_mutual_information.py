...
def test_1(dist, backend):
    ...
    """
    mimi = minimal_intrinsic_total_correlation(dist, [[0], [1]], [2], bounds=(3,), niter=100, backend=backend)
...
def test_2(dist, backend):
    ...
    Test against known values.
    """
    mimi = minimal_intrinsic_dual_total_correlation(dist, [[0], [1]], [2], bounds=(3,), niter=100, backend=backend)
    assert mimi == pytest.approx(dist.secret_rate, abs=1e-2)
...
def test_3(dist, backend):
    ...
    Test against known values.
    """
    mimi = minimal_intrinsic_CAEKL_mutual_information(dist, [[0], [1]], [2], bounds=(3,), niter=100, backend=backend)
    assert mimi == pytest.approx(dist.secret_rate, abs=1e-2)
