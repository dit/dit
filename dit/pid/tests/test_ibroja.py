...
def test_ibroja1():
    ...
    assert uniques[(0,)] == pytest.approx(0, abs=1e-5)
    assert uniques[(1,)] == pytest.approx(0, abs=1e-5)
...
def test_ibroja3():
    ...
    d = bivariates['cat']
    uniques = i_broja(d, ((0,), (1,)), (2,))
    assert uniques[(0,)] == pytest.approx(1, abs=1e-5)
    assert uniques[(1,)] == pytest.approx(1, abs=1e-5)
...
def test_pid_broja1():
    ...
    assert pid[((0,), (1,))] == pytest.approx(0.12255624891826589, abs=1e-5)
    assert pid[((0,),)] == pytest.approx(0.18872187554086706, abs=1e-5)
    assert pid[((1,),)] == pytest.approx(0.18872187554086706, abs=1e-5)
    assert pid[((0, 1),)] == pytest.approx(0.0, abs=1e-5)
...
def test_pid_broja2():
    ...
    for atom in pid._lattice:
        if atom == ((0,), (1,), (2,)):
            assert pid[atom] == pytest.approx(0.31127812445913305, abs=1e-5)
        elif atom == ((0, 1), (1, 2)):
            assert pid[atom] == pytest.approx(0.5, abs=1e-5)
        else:
