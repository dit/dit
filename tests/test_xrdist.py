...
class TestInit:
    ...
    def test_free_only(self):
        arr = xr.DataArray(np.array([[0.25, 0.25],
                                      [0.25, 0.25]]),
                           dims=['X', 'Y'], coords={'X': [0, 1], 'Y': [0, 1]})
...
class TestFromFactors:
    ...
    def test_chain_rule(self):
        ...
        np.testing.assert_allclose(joint.data.values, p_xy.data.values,
                                   atol=1e-12)
...
class TestConditionOn:
    ...
    def test_conditional_sums_to_1(self):
        ...
        np.testing.assert_allclose(
            float(c.data.sel(X=0).sum()), 1.0, atol=1e-12
        )
        np.testing.assert_allclose(
            float(c.data.sel(X=1).sum()), 1.0, atol=1e-12
        )
...
class TestMultiplication:
    ...
    def test_chain_rule(self):
        ...
        np.testing.assert_allclose(result.data.values, p_xy.data.values,
                                   atol=1e-12)
    ...
    def test_three_way_chain(self):
        ...
        assert result.given_vars == frozenset()
        np.testing.assert_allclose(result.data.values, p_xyz.data.values,
                                   atol=1e-12)
    ...
    def test_unsatisfied_given_passthrough(self):
        ...
        arr_cond = np.array([[0.5, 0.5],
                              [0.5, 0.5]])
        c = XRDistribution.from_array(
            arr_cond, ['Z', 'W'], [[0, 1], [0, 1]],
            free_vars={'Z'}, given_vars={'W'},
        )
        p = _make_pxy()
        result = p * c
        assert result.free_vars == frozenset({'X', 'Y', 'Z'})
...
class TestDivision:
    ...
    def test_conditioning_by_division(self):
        ...
        np.testing.assert_allclose(result.data.values, expected.data.values,
                                   atol=1e-12)
    ...
    def test_roundtrip_mul_div(self):
        ...
        np.testing.assert_allclose(reconstructed.data.values,
                                   p_xy.data.values, atol=1e-12)
...
class TestCopyConversion:
    ...
    def test_copy_with_base(self):
        ...
        p2 = p.copy(base=2)
        assert p2.is_log()
        np.testing.assert_allclose(p2.ops.exp(p2.data.values),
                                   p.data.values, atol=1e-12)
...
class TestLogBase:
    ...
    def test_set_base_noop(self):
        p = _make_pxy()
        p.set_base('linear')
        np.testing.assert_allclose(p.data.values,
                                   np.array([[0.1, 0.2], [0.3, 0.4]]))
    ...
    def test_marginal_in_log_space(self):
        ...
        np.testing.assert_allclose(p_x.ops.exp(p_x.data.values),
                                   expected, atol=1e-12)
